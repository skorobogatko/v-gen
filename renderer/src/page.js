...
async function init() {
  ...
  // загрузка — делаем устойчивой: не прерываем инициализацию при падении одного ресурса
  // загружаем все изображения и видео параллельно и фиксируем успешные
  ...
  // Process results: each group (settled[i].value) is an array of { src, value } or { src, error }
  try {
    if (settled[0] && settled[0].status === "fulfilled") {
      const imgs = settled[0].value;
      for (const r of imgs) {
        if (r && r.value) {
          res.images.set(r.src, r.value);
        } else {
          console.warn(
    ...
    if (settled[1] && settled[1].status === "fulfilled") {
      const logos = settled[1].value;
      for (const r of logos) {
        if (r && r.value) {
          res.logos.set(r.src, r.value);
        } else {
          console.warn(
    ...
    if (settled[2] && settled[2].status === "fulfilled") {
      const vids = settled[2].value;
      for (const r of vids) {
        if (r && r.value) {
          res.videos.set(r.src, r.value);
        } else {
          console.warn(
  ...
  // resource index is available via getResourceIndex below

  // экспонируем API для Puppeteer
  window.__renderer = {
    async renderFrame(ms) {
      // debug: list active objects and availability
      const activeListDebug = [];
      // очищаем
      ctx.fillStyle = background || "#000";
      ctx.fillRect(0, 0, width, height);

      // вычислим видимые элементы
      ...
      // если ничего не доступно для отрисовки — нарисуем временный фоллбек (чтобы не было пустых фреймов)
      const anyHas = activeListDebug.some((a) => a.has);
      if (!anyHas) {
        // draw a visible fallback so resulting frames aren't empty
      ...
      for (const o of activeObjects) {
        ...
        if (o.type === "image") {
          ...
          if (v) {
            ...
            // маленькая защита от лишних seek'ов
          ...
          const th =
            parseInt(font.match(/(\d+)px/)?.[1] || "42", 10) + pad * 1.5;
          // фон
          if (style.bg) {
            const x = o.anchor?.startsWith("center")
              ? o.x - (tw / 2 + pad)
              : o.x;
          ...
          ctx.fillText(
            text,
            o.anchor?.startsWith("center") ? o.x : o.x + pad,
            ty
          );
        }
        ctx.restore();
  ...
  // expose resource index for Puppeteer to inspect
  try {
